...
namespace Busra
{
    ...
    string CPUProgram::operator[](int index)
    {
        ...
        if(index >= strLine.size() ){
            cerr << "HATAAA !!! index line sayisindan fazla olamaz." << endl;
            return "";
        }
        else if( index < 0){
            cerr << "index negatif sayi olamaz." << endl;
            return "";
        }
        else
    ...
    //        cout << strLine[i] << endl;
        ...
        //ERROR CHECK
        if(num1 < 0   || num2 < 0)
            cout << "HATA!!! Function callda alinan parametreler negatif sayi olamaz" << endl;
        else if(size() < num2)
